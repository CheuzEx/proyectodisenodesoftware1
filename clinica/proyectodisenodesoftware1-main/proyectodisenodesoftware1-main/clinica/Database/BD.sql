...
BEGIN
  IF (TG_OP = 'DELETE') THEN
    ...
    detalle_txt := 'DELETE: ' || old_j::text;
    INSERT INTO clinica.audit_log (operacion, tabla, id_registro, detalle)
    VALUES (TG_OP, TG_TABLE_NAME, pk_val::bigint, detalle_txt);
    RETURN OLD;
  ELSIF (TG_OP = 'INSERT') THEN
    new_j := to_jsonb(NEW);
    pk_val := (new_j ->> TG_ARGV[0]);
    detalle_txt := 'INSERT: ' || new_j::text;
    INSERT INTO clinica.audit_log (operacion, tabla, id_registro, detalle)
    VALUES (TG_OP, TG_TABLE_NAME, pk_val::bigint, detalle_txt);
    RETURN NEW;
  ELSE
    ...
    detalle_txt := 'UPDATE old: ' || old_j::text || ' new: ' || new_j::text;
    INSERT INTO clinica.audit_log (operacion, tabla, id_registro, detalle)
    VALUES (TG_OP, TG_TABLE_NAME, pk_val::bigint, detalle_txt);
    RETURN NEW;
  END IF;
...
-- audit_log uses bigserial (auto sequence)
...
-- audit_log id_audit is bigserial
...
COPY clinica.audit_log (tabla, id_registro, operacion, detalle) FROM stdin;
\.
